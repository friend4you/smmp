...
final class NewPostCoordinator: Coordinating, ObservableObject {
    ...
    fileprivate func newPostRoot() -> NewPostView {
        builder.buildNewPost(onPostCreated: handlePostCreated)
    }

    @ViewBuilder
    fileprivate func destination(for route: NewPostRoute) -> some View {
